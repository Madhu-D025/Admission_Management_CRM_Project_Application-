 ...
 * FeeManagement
 *
 * Identical UI structure to DocumentVerification.
 * Approve calls UpdateApplicantFormFeeStatus
 * with feeStatus = "Paid" — value never shown in UI.
 */
  ...
  const handleApprove = async () => {
    ...
    try {
      const payload = {
        ...
        feeStatus:   "Paid",                  // hidden from UI — hardcoded as per spec
        userId:      personalInfo.userID,
      };

      const res = await axios.post(
        `${config.API_URL}ApplicantForm/UpdateApplicantFormFeeStatus`,
        payload
      );

      if (res.data.success) {
        toast.success(res.data.message || "Fee status updated successfully");
        handleCloseModal();
    ...
    <section className="content">
      <div className="card">
        <div className="card-header">
          <h5 className="card-title mb-0">Fee Management</h5>
        </div>
